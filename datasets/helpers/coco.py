...
class CocoSegmentation(data.Dataset):
    def __init__(self,
        ...
        if mini:
            self.coco.imgs = {x: self.coco.imgs[x] for x in list(
                np.sort(self.coco.imgs.keys())[:5000])}

        if images_with_all_cstm_cats is not None:
            self.img_ids = self.coco.getImgIds(
                imgIds=self.coco.imgs.keys(), catIds=cat_ids)
        else:
            self.img_ids = self.coco.imgs.keys()

        self.ids = self.coco.getAnnIds(
            imgIds=self.img_ids, areaRng=area_range, catIds=cat_ids)
    ...
    def __getitem__(self, index):
        ...
        if not self.return_rle:
            try:
                img = np.array(Image.open(os.path.join(self.root, path)).convert(
                ...
                if self.one_mask_per_class:
                    ...
                    for ii in range(len(cat_id)):
                        ...
                        for ann in ann_meta_class:
                            target_tmp = np.logical_or(
                                target_tmp, np.array(coco.annToMask(ann)))
                        target[target_tmp] = ii+1
                    target = target[:, :, np.newaxis]
                else:
                    target = np.zeros([img.shape[0], img.shape[1], 1])
                    for ann in ann_meta:
                        target = np.logical_or(target, np.array(coco.annToMask(
                            ann).reshape([img.shape[0], img.shape[1], 1])))
                target = target.astype(np.float32)
            except Exception:
                img = np.zeros((100, 100, 3))
                target = np.zeros((100, 100))
                print('Error reading image '+str(path) +
                      ' with object id '+str(self.ids[index]))
    ...
    def __str__(self):
        return 'CocoSegmentation(split='+str(self.split)+', area_range='+str(self.area_range) +\
               ...
               ', only_pascal_categories=' + \
            str(self.only_pascal_categories)+')'
