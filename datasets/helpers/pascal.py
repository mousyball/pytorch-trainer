...
class VOCSegmentation(data.Dataset):
    ...
    def __getitem__(self, index):
        _img, _target, _void_pixels, _, _, _ = self._make_img_gt_point_pair(
            index)
        ...
        if self.default:
            ...
            for ii in range(1, np.max(_tmp).astype(np.int)+1):
                ...
                if obj_cat == ii_cat and ii != _obj_ii+1:
                    _other_same_class = np.logical_or(
                        _other_same_class, _tmp == ii)
                elif ii != _obj_ii+1:
...
if __name__ == '__main__':
    ...
    dataset = VOCSegmentation(
        split=['train', 'val'], transform=transform, retname=True)
