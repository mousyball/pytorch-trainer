...
class CombineDBs(data.Dataset):
    def __init__(self, dataloaders, excluded=None):
        ...
        self.logger.info('{0:6s} Images: {1:5d}, Objects: {2:5d}'.format(
...
if __name__ == "__main__":
    ...
    pascal_voc_val = pascal.VOCSegmentation(
        split='val', transform=transform, retname=True)
    sbd = sbd.SBDSegmentation(
        split=['train', 'val'], transform=transform, retname=True)
    pascal_voc_train = pascal.VOCSegmentation(
        split='train', transform=transform, retname=True)
    ...
    for ii, sample in enumerate(dataloader):
        for jj in range(sample["image"].size()[0]):
            ...
            overlay = helpers.overlay_mask(helpers.tens2image(
                sample["image"][jj])/max_img, helpers.tens2image(sample["gt"][jj]))
            plt.imshow(overlay)
