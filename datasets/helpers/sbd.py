...
# [issue] [OSError: cannot identify image file png](https://github.com/python-pillow/Pillow/issues/3287)
ImageFile.LOAD_TRUNCATED_IMAGES = True


class SBDSegmentation(data.Dataset):
    logger = logging.getLogger("[SBD]")
    ...
    def __init__(self,
        ...
        self.dataset_dir = os.path.join(
            self.root, 'benchmark_RELEASE', 'dataset')
        ...
        # Display stats
        self.logger.info('{0:10s} Images: {1:5d}, Objects: {2:5d}'.format(
            '_'.join(self.split), num_images, len(self.obj_list)))
    ...
    def _check_integrity(self):
        ...
        if not os.path.isfile(_fpath):
            self.logger.error("{} does not exist".format(_fpath))
            return False
        _md5c = hashlib.md5(open(_fpath, 'rb').read()).hexdigest()
        if _md5c != self.MD5:
            self.logger.error(" MD5({}) did not match MD5({}) expected for {}".format(
    ...
    def _preprocess(self):
        ...
        self.logger.info('Pre-processing finished')
    ...
    def _download(self):
        ...
        if self._check_integrity():
            ...
            self.logger.info('Downloading ' + self.URL + ' to ' + _fpath)
        ...
        # extract file
        cwd = os.getcwd()
        self.logger.info('Extracting tar file')
    ...
    def _make_img_gt_point_pair(self, index):
        ...
        # Read Image
        _img = np.array(Image.open(self.images[_im_ii]).convert(
            'RGB')).astype(np.float32)

        # Read Taret object
        _tmp = scipy.io.loadmat(self.masks[_im_ii])[
            "GTinst"][0]["Segmentation"][0]
...
if __name__ == '__main__':
    ...
    for i, _data in enumerate(dataloader):
        plt.figure()
        plt.imshow(helpers.tens2image(_data['image'])/255)
        plt.figure()
        plt.imshow(helpers.tens2image(_data['gt'])[:, :, 0])
        if i == 10:
            break
