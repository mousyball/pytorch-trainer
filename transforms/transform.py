...
class CustomCropFromMask(BaseTransform):
    ...
    def __call__(self, sample) -> dict:
        ...
        for input_key, output_key in zip(self._input_key, self._output_key):
            _crop = []
            traget = sample[input_key]
            if traget.ndim == 2:
                traget = np.expand_dims(traget, axis=-1)

            for k in range(0, _mask.shape[-1]):
                if np.max(_mask[..., k]) == 0:
                    _crop.append(np.zeros(traget.shape, dtype=traget.dtype))
                else:
                    _crop.append(
                        dextr_helper.crop_from_mask(traget,
                                                    _mask[..., k],
                                                    relax=self._relax,
