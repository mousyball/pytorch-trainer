...
import cv2
import numpy as np
import pycocotools.mask as coco_maskUtils
...
def crop2fullmask(crop_mask, bbox, im=None, im_size=None, zero_pad=False, relax=0, mask_relax=True,
    ...
    if scikit:
        ...
        crop_mask = cv2.resize(
            crop_mask,
            (bbox[2] - bbox[0] + 1, bbox[3] - bbox[1] + 1), interpolation=interpolation
        )
...
def overlay_mask(im, ma, colors=None, alpha=0.5):
    ...
    if ma.ndim == 2:
        fg = im * alpha \
            + np.ones(im.shape) * (1 - alpha) \
            * colors[1, :3]   # np.array([0,0,255])/255.0
    else:
        fg = []
        for n in range(ma.ndim):
            fg.append(im * alpha + np.ones(im.shape)
                      * (1 - alpha) * colors[1+n, :3])
    # Whiten background
    bg = im.copy()
    if ma.ndim == 2:
        bg[ma == 0] = im[ma == 0]
...
def overlay_masks(im, masks, alpha=0.5):
    ...
    for ma in masks:
        ...
        fg = im * alpha \
            + np.ones(im.shape) * (1 - alpha) \
            * colors[i, :3]   # np.array([0,0,255])/255.0
        i = i + 1
        ov[ma == 1] = fg[ma == 1]
        total_ma += ma
...
def extreme_points(mask, pert):
    ...
    return np.array([
        find_point(inds_x, inds_y, np.where(
            inds_x <= np.min(inds_x)+pert)),  # left
        find_point(inds_x, inds_y, np.where(
...
def crop_from_bbox(img, bbox, zero_pad=False):
    ...
    if zero_pad:
        ...
        crop = np.zeros((bbox_valid[3] - bbox_valid[1] + 1,
    ...
    if img.ndim == 2:
        crop[inds[1]:inds[3] + 1, inds[0]:inds[2] + 1] = \
            img[bbox_valid[1]:bbox_valid[3] + 1,
                bbox_valid[0]:bbox_valid[2] + 1]
    else:
...
def fixed_resize(sample, resolution, flagval=None):
    ...
    if sample.ndim == 2 or (sample.ndim == 3 and sample.shape[2] == 3):
        ...
        tmp = sample
        sample = np.zeros(
            np.append(resolution, tmp.shape[2]),
            dtype=np.float32
        )
...
def crop_from_mask(img, mask, relax=0, zero_pad=False):
    if mask.shape[:2] != img.shape[:2]:
        mask = cv2.resize(
            mask,
            dsize=tuple(reversed(img.shape[:2])),
...
def mask_2_rle(mask):
    ...
    if len(mask.shape) == 2:
        rle = coco_maskUtils.encode(mask)
    if len(mask.shape) == 3:
        rle = coco_maskUtils.encode(mask)[0]

    rle['counts'] = rle['counts'].decode('ascii')
    return rle


def ann_2_mask(ann, shape):
    """
    ...
    if type(segm) == list:
        ...
        rles = coco_maskUtils.frPyObjects(segm, h, w)
        rle = coco_maskUtils.merge(rles)
    elif type(segm['counts']) == list:
        # uncompressed RLE
        rle = coco_maskUtils.frPyObjects(segm, h, w)
    else:
        # rle
        rle = ann['segmentation']

    mask = coco_maskUtils.decode(rle)
    return mask
...
def toBbox(rle):
    return coco_maskUtils.toBbox(rle)
